...
for sample in sample_dirs_single:
    ...
    sample_counts_single = abundance_df_single[['target_id', 'tpm']].copy()
    
    # tpm = name of sample
    sample_counts_single.rename(columns={'tpm': sample}, inplace=True)
...
for sample in sample_dirs_paired:
    ...
    # extract target ID (1st column) and tpm
    sample_counts_paired = abundance_df_paired[['target_id', 'tpm']].copy()
    
    # tpm = name of sample
    sample_counts_paired.rename(columns={'tpm': sample}, inplace=True)
...
# save as csv file
count_matrix.to_csv("/dss/dssfs03/pn57ba/pn57ba-dss-0001/computational-plant-biology/luisa/bachelor-thesis-luisa/count_tables/counts_tpm.csv")
