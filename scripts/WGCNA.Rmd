...
plotDendroAndColors(geneTree_pow4_tomsigned, 
                    moduleColors_pow4_tomsigned,
                    "Module",
                    dendroLabels = FALSE, 
                    hang = 0.03,
                    addGuide = TRUE, 
...
table(moduleColors_pow4_tomsigned)
```
...
picked_module_colors = moduleColors_pow4_tomsigned
...
metaData_bin <- read.csv("~/Studium/bachelor-thesis-luisa/data/metaData_bin.csv", sep = ";",row.names = 1)
metaData_bin <- binarizeCategoricalColumns(metaData_bin, convertColumns = "latest_infection_timepoint", includePairwise = FALSE, includeLevelVsAll = TRUE, minCount = 1, dropFirstLevelVsAll = FALSE)
...
abs(geneTraitSignificance_sub_cat[moduleGenes,category]),
...
```
KEGG enriched pathways (for hub genes)
royalblue: Plant-pathogen interaction - Arabidopsis thaliana (thale cress)
darkred: Plant-pathogen interaction - Arabidopsis thaliana (thale cress),MAPK signaling pathway - plant - Arabidopsis thaliana (thale cress),Glycerolipid metabolism - Arabidopsis thaliana (thale cress)
black: Plant-pathogen interaction - Arabidopsis thaliana (thale cress)
yellow: Pentose and glucuronate interconversions - Arabidopsis thaliana (thale cress)
darkgrey: Oxidative phosphorylation - Arabidopsis thaliana (thale cress)
greenyellow: Starch and sucrose metabolism - Arabidopsis thaliana (thale cress)
...
simple_ego %>% filter(p.adjust < 0.05) %>%
  ...
  xlab("Gene Ratio") +
  ylab(NULL) + 
  ggtitle(paste("GO Enrichment of hub genes of royalblue module"))
...
```{r}
#bitr(hub_genes_oomycete_royalblue$GeneID, fromType="TAIR", toType="ENTREZID", OrgDb="org.At.tair.db")
kegg <- enrichKEGG(gene = hub_genes_nematode_brown$GeneID,
    organism = "ath", 
    pAdjustMethod = "BH",
    pvalueCutoff = 0.05)
...
#browseKEGG(kegg, 'ath01230')

#library("pathview")
#ath01230 <- pathview(gene.data  = hub_data_biotroph_darkred$GeneID,
 #                    pathway.id = "ath01230",
  #                   species    = "ath")
```
